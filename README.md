...
## 启动机制和注解驱动
### 需求
当前我们的 RPC 框架已经较为完善，接下来从使用者的角度来考虑，优化 RPC 框架。

框架选择关注以下几点：
1. 知名度和用户数
2. 生态和社区活跃度
3. 简单易用易上手：最好能开箱即用，这点可能是我们在做个人小型轮子项目时最关注的。✔

从第三点出发，当前我们 RPC 框架并不够易用，从 example-provider 就看的出来，需要一套又臭又长的代码来使用 RPC 框架：

初始化→读取配置→构建服务信息对象→获取注册中心实例→注册服务到注册中心→启动 web 服务

**目标**：优化框架的易用性，通过建立合适的启动机制和注解驱动机制，帮助开发者最少只用一行代码，就能轻松使用框架！
### 设计
#### 启动机制设计
把所有的启动代码封装成一个 **专门的启动类** 或方法，然后由服务提供者 / 服务消费者调用即可。

但是服务提供者和服务消费者**需初始化的模块不同**，比如服务消费者无需启动 Web 服务器。

因此，我们需要正对服务提供者和消费者分别编写一个启动类，如果是二者都需要初始化的模块，可以放到全局应用类 ***`RpcApplication`*** 中。

可参考 Dubbo：https://cn.dubbo.apache.org/zh-cn/overview/mannual/java-sdk/quick-start/api/
#### 注解驱动设计
更主流更方便的使用框架的方法 - **注解驱动**
参考 Dubbo，开发者只需在服务提供者实现类上写一个 ***`@DubboService`*** 注解，就能快速**注册**服务；
同样的，只要在服务消费者字段上写一个 ***`@DubboReference`*** 注解，就能快速**获取**服务。

由于现在的 Java 项目基本都使用 Spring Boot 框架，所以可创建一个 ***Spring Boot Starter*** 项目，并开通注解驱动框架的优化，完成服务注册和获取引用。
